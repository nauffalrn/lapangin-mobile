...
class Booking {
  ...
  // Tambahkan properti ini di class Booking
  final String? jamMulaiFormatted;
  final String? jamSelesaiFormatted;
  ...
  Booking({
    ...
    this.jamMulai,
    this.jamSelesai,
    this.jamMulaiFormatted,
    this.jamSelesaiFormatted,
  });
  ...
  Map<String, dynamic> toJson() {
    ...
    List<Map<String, dynamic>> individualSlots =
        jadwalList.map((item) {
          return {
            'jam': item.jam,
            'jamMulai': item.jam,
            ...
            'harga': item.harga,
            'waktu':
                "${item.jam.toString().padLeft(2, '0')}:00-${(item.jam + 1).toString().padLeft(2, '0')}:00",
          };
        }).toList();
  ...
  factory Booking.fromJson(Map<String, dynamic> json) {
    ...
    if (json['jamMulai'] != null && json['jamSelesai'] != null) {
      jamMulai =
          json['jamMulai'] is int
              ? json['jamMulai']
              : int.tryParse(json['jamMulai'].toString()) ?? 0;
      jamSelesai =
          json['jamSelesai'] is int
              ? json['jamSelesai']
              : int.tryParse(json['jamSelesai'].toString()) ?? 0;
      waktu =
    ...
    if (json['lapangan'] != null) {
      if (json['lapangan'] is Map) {
        ...
        lokasi =
            lapanganData['alamatLapangan']?.toString() ??
    ...
    // Parse jam formatted
    String? jamMulaiFormatted = json['jamMulaiFormatted']?.toString();
    String? jamSelesaiFormatted = json['jamSelesaiFormatted']?.toString();

    // Tambahkan parsing untuk review
    int? rating;
    String? review;
    String? reviewerName;
    String? reviewDate;

    // Parse review data jika ada
    if (json['review'] != null) {
      var reviewData = json['review'];
      if (reviewData != null) {
        if (reviewData['rating'] != null) {
          rating =
              reviewData['rating'] is int
                  ? reviewData['rating']
                  : int.tryParse(reviewData['rating'].toString());
        }
        review = reviewData['komentar']?.toString();
        reviewerName = reviewData['username']?.toString();
        reviewDate = reviewData['tanggalReview']?.toString();
      }
    }

    print(
      "Parsed booking - ID: $id, Tanggal: $tanggal, Waktu: $waktu, Venue: $namaLapangan",
    );
    ...
    return Booking(
      ...
      review: review, // Tambahkan ini
      reviewerName: reviewerName, // Tambahkan ini
      reviewDate: reviewDate, // Tambahkan ini
