...
class BookingService {
  ...
  static String formatDateForBackend(String originalDate) {
    ...
    try {
      ...
      if (parts.length == 3) {
        ...
        final month = (months.indexOf(monthName) + 1).toString().padLeft(2, '0');
  ...
  static Future<Booking> createBooking(Booking booking) async {
    try {
      ...
      if (booking.jadwalList.isNotEmpty) {
        ...
        booking.waktu = "${earliestJam.toString().padLeft(2, '0')}:00-${latestJam.toString().padLeft(2, '0')}:00";
      }
      ...
      final Map<String, dynamic> requestBody = {
        ...
        'jadwalList': booking.jadwalList.map((item) => {
          'jam': item.jam,
          'harga': item.harga,
        }).toList(),
      };
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is Map && responseData.containsKey('success')) {
          ...
          if (bookingData is int || (bookingData is String && int.tryParse(bookingData) != null)) {
            // Create a booking object with ID but copy other fields from request
            int bookingId = bookingData is int ? bookingData : int.parse(bookingData);
            ...
            if (result.waktu == null && booking.jadwalList.isNotEmpty) {
              ...
              result.waktu = "${earliestJam.toString().padLeft(2, '0')}:00-${latestJam.toString().padLeft(2, '0')}:00";
        ...
        if (result.waktu == null && booking.jadwalList.isNotEmpty) {
          ...
          result.waktu = "${firstHour.toString().padLeft(2, '0')}:00-${lastHour.toString().padLeft(2, '0')}:00";
        ...
        print("Created and saved booking ID ${result.id}, with waktu: ${result.waktu}");
        return result;
        ...
        if (data['success'] == true && data['data'] != null) {
          ...
          for (int i = 0; i < bookingsData.length; i++) {
            try {
              ...
              if (processed['waktu'] == null && processed['jamMulai'] != null && processed['jamSelesai'] != null) {
                int jamMulai = processed['jamMulai'] is int 
                    ? processed['jamMulai'] 
                    : int.tryParse(processed['jamMulai'].toString()) ?? 0;
                    
                int jamSelesai = processed['jamSelesai'] is int 
  ...
  static Future<void> saveBookingLocally(Booking booking) async {
    try {
      ...
      if (booking.jadwalList.isNotEmpty) {
        cleanData['jadwalList'] = booking.jadwalList.map((item) => {
          'jam': item.jam,
          'harga': item.harga,
        }).toList();
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is Map && responseData.containsKey('success')) {
          if (responseData['success'] == true && responseData['data'] != null) {
            ...
            if (booking.waktu == null && booking.jamMulai != null && booking.jamSelesai != null) {
              String startTime = "${booking.jamMulai.toString().padLeft(2, '0')}:00";
              String endTime = "${booking.jamSelesai.toString().padLeft(2, '0')}:00";
            ...
            throw Exception(responseData['message'] ?? 'Failed to load booking details');
          }
        } else {
  ...
  static Booking parseJsonToBooking(Map<String, dynamic> json) {
    ...
    if (json['jamMulai'] != null) {
      jamMulai = json['jamMulai'] is int ? json['jamMulai'] : int.tryParse(json['jamMulai'].toString());
    }
    
    if (json['jamSelesai'] != null) {
      jamSelesai = json['jamSelesai'] is int ? json['jamSelesai'] : int.tryParse(json['jamSelesai'].toString());
    }
  ...
  static Map<String, dynamic> _preprocessBookingData(Map<String, dynamic> rawData) {
      ...
      if (dateStr.contains(":")) {
        try {
          ...
          if (dateStr.contains("T")) {
            ...
            processed['jamSelesai'] = processed['jamMulai'] + 1; // Assuming 1-hour slots
          }
